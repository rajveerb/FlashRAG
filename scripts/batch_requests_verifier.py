# python scripts/batch_requests_verifier.py --prompt_file_dir profile_logs --prompt_file_prefix past_generation_result_iter --config_path examples/methods/my_config.yaml --output_dir profile_logs/IterativePipeline/batch_requests/ --batch_size 256
...
def main():
    ...
    output_dir = os.path.join(args.output_dir, str(args.batch_size))
    ...
    for prompt_file in prompt_files:
        ...
        with open(prompt_file, 'r', encoding="utf-8") as f:
            ...
            for i in tqdm(range(0, len(prompts), batch_size)):
                ...
                outputs = generator.generate(batch_prompts, return_raw_output=True)
                end_time = time.time()
                ...
                batch_output[i] = (stored_output, start_time, end_time - start_time)
                
        # log batch generation time
        open(os.path.join(output_dir,f"generation_time_iter_{iter_idx}.txt"), "w").write(json.dumps(batch_output))
